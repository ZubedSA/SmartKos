...
export default function Sidebar({ items, title, user, onLogout, isOpen, onClose }) {
    ...
    return (
        <>
            ...
            <aside
                className={`fixed left-0 top-0 h-full w-64 bg-[#1e293b] border-r border-[#334155] z-50 flex flex-col transform transition-transform duration-300 lg:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"
                    }`}
            >
                {/* Logo & Close Button */}
                    ...
                    {items.map((item) => {
                        ...
                        return (
                            <Link
                                key={item.href}
                                href={item.href}
                                onClick={() => onClose && onClose()} // Close sidebar on navigate
